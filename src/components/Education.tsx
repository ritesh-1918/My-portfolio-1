    ...
    <section id="education" ref={containerRef} className="min-h-screen py-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="relative">
          {/* Central timeline line with enhanced visual */}
          <div className="absolute left-1/2 transform -translate-x-1/2 h-full w-1 bg-gradient-to-b from-primary via-secondary to-primary rounded-full shadow-lg"></div>
          
          {/* Enhanced timeline dots with pulse animation */}
          {educationData.map((item, index) => (
            <div 
              key={`dot-${item.id}`} 
              className="absolute left-1/2 transform -translate-x-1/2 w-5 h-5 rounded-full bg-primary border-4 border-background z-10"
          ...
          {educationData.map((item, index) => (
            ...
            >
              ...
              <div className={`w-full lg:w-5/12 mt-8 lg:mt-0 ${index % 2 === 0 ? 'lg:mr-auto lg:pr-8' : 'lg:ml-auto lg:pl-8'} relative overflow-hidden`}>
                {/* Enhanced year badge with subtle animation */}
                <motion.div 
                  className="absolute -top-3 left-1/2 transform -translate-x-1/2 z-20"
                  whileHover={{ scale: 1.05 }}
                  transition={{ type: "spring", stiffness: 300 }}
                >
                  <div className="bg-gradient-to-r from-primary to-secondary text-white py-2 px-4 rounded-full text-center font-medium shadow-lg border-2 border-background">
                    {item.year}
                  </div>
                </motion.div>
                ...
                {applyGlass(
                  <motion.div 
                    className="relative pt-10 pb-6 cursor-pointer"
                    whileHover={{ y: -5, transition: { duration: 0.2 } }}
                    style={{ scale: cardScale, rotateY: rotateY }}
                  >
                    <div className="p-6">
                  ...
                  { intensity: 15, borderRadius: '1rem', depth: 3, className: 'h-full overflow-hidden' }
                )}
              </div>
            </motion.div>
          ))}
